import { getTemplateById, getTemplateByMap, resolveTemplateVersion } from './templates.js';
...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--skip-install') { result.skipInstall = true; continue; }
    if (arg === '--no-git') { result.git = false; continue; }
    if (arg === '--git') { result.git = true; continue; }

    const [key, inlineValue] = arg.startsWith('--') && arg.includes('=')
      ? arg.split(/=(.*)/s, 2)
      : [arg, null];

    if (['--template', '--template-version', '--map', '--package-manager'].includes(key)) {
      const value = inlineValue ?? argv[index + 1];
      if (!value || value.startsWith('--')) throw new Error(`${key} 옵션에 값이 필요합니다.`);
      if (inlineValue === null) index += 1;

      if (key === '--template') {
        const at = value.lastIndexOf('@');
        if (at > 0) {
          result.template = value.slice(0, at);
          result.templateVersion = value.slice(at + 1);
        } else {
          result.template = value;
        }
      }
      if (key === '--template-version') result.templateVersion = value;
  ...
  if (positional.length > 1) throw new Error('프로젝트 경로는 하나만 지정할 수 있습니다.');
...
export function resolveTemplateFromArgs(args) {
  ...
  if (args.template && !byId) throw new Error(`존재하지 않는 템플릿입니다: ${args.template}`);

  const byMap = args.map ? getTemplateByMap(args.map) : null;
  if (args.map && !byMap) throw new Error(`지원하지 않는 지도 설정입니다: ${args.map}`);

  if (byId && byMap && byId.id !== byMap.id) {
    throw new Error(`--template ${byId.id} 와 --map ${byMap.map} 설정이 서로 충돌합니다.`);
  }

  const selected = byId ?? byMap ?? null;
  return selected ? resolveTemplateVersion(selected, args.templateVersion) : null;
}
