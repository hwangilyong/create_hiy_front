...
const AI_REVIEW_DEMO_TEMPLATE_DIR = path.join(MODULE_DIR, '..', 'templates', 'ai-review-demo');
...
function cloneTemplate(template, cloneDir) {
  const failures = [];
  if (commandExists('gh')) {
    const result = cloneWith('gh', ['repo', 'clone', template.repository, cloneDir, '--', '--depth', '1', '--branch', template.ref]);
    if (result.status === 0) return;
    failures.push(`gh: ${(result.stderr || result.stdout || '').trim()}`);
    removeIfExists(cloneDir);
  }
  if (!commandExists('git')) throw new Error('git 명령을 찾을 수 없습니다. Git을 설치한 뒤 다시 실행해주세요.');
  const result = cloneWith('git', ['clone', '--depth', '1', '--branch', template.ref, `https://github.com/${template.repository}.git`, cloneDir], {
    env: { ...process.env, GIT_TERMINAL_PROMPT: '0' },
  });
  if (result.status === 0) return;
  failures.push(`git: ${(result.stderr || result.stdout || '').trim()}`);
  const detail = failures.filter(Boolean).join('\n').slice(-1600);
  throw new Error(`템플릿 ${template.repository} 을(를) 가져오지 못했습니다. 현재 템플릿 저장소가 private이면 \`gh auth login\` 또는 Git credential 설정이 필요합니다.${detail ? `\n\n${detail}` : ''}`);
}
...
function packageNameFromPath(targetDir) {
  ...
  return raw.replace(/[\s_]+/g, '-').replace(/[^a-z0-9._-]/g, '-').replace(/^[._-]+/, '').replace(/-+/g, '-') || 'hiy-front-app';
}
...
function configureStorybookAiReview(targetDir) {
  const packageJsonPath = path.join(targetDir, 'package.json');
  if (!existsSync(packageJsonPath)) throw new Error('Storybook AI Review를 구성하려면 package.json이 필요합니다.');
  const packageJson = JSON.parse(readFileSync(packageJsonPath, 'utf8'));
  packageJson.scripts = { ...(packageJson.scripts ?? {}), storybook: 'storybook dev -p 6006', 'build-storybook': 'storybook build', 'ai-bridge': 'node tools/ai-bridge/server.mjs' };
  packageJson.devDependencies = { ...(packageJson.devDependencies ?? {}), '@hiy/storybook-addon-ai-review': 'github:hwangilyong/storybook_addon#main', '@storybook/react-vite': 'latest', 'better-sqlite3': '^12.2.0', storybook: 'latest' };
  writeFileSync(packageJsonPath, `${JSON.stringify(packageJson, null, 2)}\n`);

  const storybookDir = path.join(targetDir, '.storybook');
  mkdirSync(storybookDir, { recursive: true });
  writeFileSync(path.join(storybookDir, 'main.ts'), `import type { StorybookConfig } from '@storybook/react-vite';\n\nconst config: StorybookConfig = {\n  framework: '@storybook/react-vite',\n  stories: ['../src/**/*.mdx', '../src/**/*.stories.@(js|jsx|mjs|ts|tsx)'],\n  addons: ['@hiy/storybook-addon-ai-review/preset'],\n};\n\nexport default config;\n`);
  writeFileSync(path.join(storybookDir, 'preview.ts'), `import type { Preview } from '@storybook/react-vite';\n\nconst preview: Preview = {\n  parameters: {\n    hiyAiReview: {\n      endpoint: import.meta.env.VITE_HIY_AI_REVIEW_ENDPOINT ?? '',\n    },\n  },\n};\n\nexport default preview;\n`);
  ...
  if (!existingEnv.includes('VITE_HIY_AI_REVIEW_ENDPOINT=')) {
    ...
    writeFileSync(envExamplePath, `${existingEnv}${prefix}VITE_HIY_AI_REVIEW_ENDPOINT=http://127.0.0.1:4700/review\n`);
  }
...
function injectAiBridge(targetDir) {
  if (!existsSync(AI_BRIDGE_TEMPLATE_DIR)) throw new Error(`ai-bridge 템플릿을 찾을 수 없습니다: ${AI_BRIDGE_TEMPLATE_DIR}`);
...
function injectAiReviewDemo(targetDir) {
  if (!existsSync(AI_REVIEW_DEMO_TEMPLATE_DIR)) throw new Error(`AI Review 데모 템플릿을 찾을 수 없습니다: ${AI_REVIEW_DEMO_TEMPLATE_DIR}`);
  const demoDir = path.join(targetDir, 'src', 'ai-review-demo');
  mkdirSync(demoDir, { recursive: true });
  cpSync(AI_REVIEW_DEMO_TEMPLATE_DIR, demoDir, { recursive: true });
}

function initializeGit(targetDir, warnings) {
  if (!commandExists('git')) { warnings.push('Git을 찾을 수 없어 새 저장소 초기화를 건너뛰었습니다.'); return; }
...
function installDependencies(targetDir, packageManager, warnings) {
  ...
  if (!commandExists(command)) { warnings.push(`${command} 명령을 찾을 수 없어 의존성 설치를 건너뛰었습니다.`); return; }
...
export function createProject({
  ...
  try {
    ...
    cpSync(cloneDir, targetDir, { recursive: true, filter(source) { return path.basename(source) !== '.git'; } });
    normalizeGeneratedProject(targetDir);
    if (storybookAiReview) configureStorybookAiReview(targetDir);
    if (storybookAiReviewDemo) injectAiReviewDemo(targetDir);
